...
def pay(request):
  if request.method == 'POST':
    ...
    cburl= 'http://54.89.180.400/callback'
    ref = str(uuid.uuid4())
    profile = Profile.objects.get(user__username = request.user.username)
    ...
    data = {'reference':ref, 'callback_url':cburl, 'email':user.email, 'amount':int(total), 'order_number':shop_code}
...
def callback(request):
  ...
  for items in trolley:
    ...
    stock.max_quantity -= items.quant
    stock.save()
...
def search(request):
  if request.method == 'POST':
    ...
    searched = Q(Q(title_r__contains=items) | Q(price__icontains=items))
